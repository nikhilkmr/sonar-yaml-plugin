...
public class SimpleYamlMatchCheckTest extends AbstractCheckTester {

    @Test
    public void simpleCase_successfulMatch() throws IOException {
	// Set up
	super.createFileSystem();
	File tempFile1 = super
		.createTempFile("objectionable string\n\nsadf\n\n1objectionable string");
	
	SimpleYamlMatchCheck check = new SimpleYamlMatchCheck();
	...
	File jsFile = new File("src" + sep + "test" + sep + "resources" + sep
		+ "invalidCharacterBytes.js");

	SimpleYamlMatchCheck check = new SimpleYamlMatchCheck();
	check.setExpression(".*(ts_sort_numeric|ts_sort_datetime).*");

	// Run
	YamlSourceFile result = parseAndCheck(jsFile, check,
		"com.mycorp.projectA.service:service-do-X");
    ...
    @Test
    public void simpleCaseInvolvingStartOfLineCharacter_usesStartOfEachLine()
	    throws IOException {
	// Set up
	super.createFileSystem();
	File tempFile1 = super
	...
	// Run
	YamlSourceFile result = parseAndCheck(tempFile1, check,
		"com.mycorp.projectA.service:service-do-SPECIAL_THING-blah-blah");
	...
	SimpleYamlMatchCheck check = new SimpleYamlMatchCheck();
	check.setExpression(".*objectionable string.*");
	check.setDoNotFireForProjectKeysRegex(".*do-SPECIAL_THING.*"); // should
								       // have
								       // no
								       // effect
								       // here
	check.setDoNotFireForTheseFileNamesRegex("file.xml"); // this is the
							      // file name that
							      // 'super.createFileSystem()'
							      // sets so this
							      // will have an
